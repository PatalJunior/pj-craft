...
const incomingData = (configdata) => {
  ...
  for (i = 0; i < configdata.length; i++) {
    text += `
        <div class="weaponlist" data-itemname = ${configdata[i].itemLabel} data-required = ${JSON.stringify(configdata[i].required)} data-weaponName = ${configdata[i].itemName} data-weaponTime = ${configdata[i].minute} data-itemlevel = ${configdata[i].level}  data-xp = ${configdata[i].xp}  >
        ...
        <div class="weaponMargin" style="background-image:url(nui://${imageFolder}${configdata[i].itemName}.png)"> 
...
$(document).on('click', '.weaponlist', function () {
  ...
  console.log(parseInt(itemlevel))
  ...
  $('.showweapons').attr("src",`nui://${imageFolder}${weaponName}.png`);
  $('.showweapons').attr("height",'90%');   
  $('.showweapons').attr("onerror",'this.onerror=null; this.remove();');
  let text2 = "";
...
$('.weaponspanminute').text(weaponMinute);
  ...
  for (i = 0; i < required2.length; i++) {     
    ...
    text2 += `
        <div class="items">
        <img class="gerekliItempng" src="nui://${imageFolder}${required2[i].name}.png" alt="">
    ...
    <img class="gerekliItempng" src="nui://${imageFolder}${required2[i].name}.png" alt="">
    <h2>${required2[i].label}</h2>
    <h3 style='color: red;'>${required2[i].amount}/0</h3>
     </div>
    `;
  }
  }
...
$(document).on('click', '.startButton', function () {
  ...
  weaponName :weaponName, weaponMinute:weaponMinute , weaponLabel:weaponLabel,itemName : gerekliitemler ,xpweapon:xpweapon
  }));
});
...
const sqlgelendata = (configdata) => {
  ...
  for (i = 0; i < configdata.length; i++) {
      text += `
      <div class="weaponlist2" data-itemname = ${configdata[i].itemLabel} data-claimitem = ${configdata[i].itemName} data-id =${configdata[i].id}  data-sqlzaman =${configdata[i].craftStartTime} data-itemzaman = ${configdata[i].craftTime}  data-weaponimage =${configdata[i].itemName} >
      <h2 class="weaponame2">${configdata[i].itemLabel}</h2>
      <div class="weaponbackground2"></div>
      <div class="weaponMargin2" style="background-image:url(nui://${imageFolder}${configdata[i].itemName}.png)"> 
...
$(document).on('click', '.weaponlist2', function () {
  ...
  $('.showweapons').attr("src",`nui://${imageFolder}${images}.png`);   
